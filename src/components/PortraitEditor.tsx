...
export default function PortraitEditor({ image, onImageChange, retired }: Props) {
  ...
  return (
    <>
      {/* Portrait display with upload button */}
      <div
        className="relative w-28 flex-shrink-0 border-l border-gray-700/50 group/img"
        onClick={e => e.stopPropagation()}
      >
        {image ? (
          <img src={image} alt="portrait" className="w-full h-full object-cover object-top" />
        ) : (
      ...
      {/* Portrait positioning modal — rendered via portal to escape backdrop-filter stacking context */}
