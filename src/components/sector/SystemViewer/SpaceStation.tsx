...
import { useLodModels, type LodEntry } from './useLodModels';
...
export default function SpaceStation({ obj, isInBelt = false, onPositionUpdate, onClick, showOrbits = true }: Props) {
  ...
  // LOD entries are keyed by subtype, which is stable for the lifetime of this component.
  const lodEntries = useMemo<LodEntry[]>(() => {
    if (subtype === 'deep') return [
      { path: null,                                                              maxDistance: 0          },
      { path: '/models/space_station_3/spacestation3_LOD1.glb',                 maxDistance: 0.5        },
      { path: '/models/space_station_3/spacestation3_LOD2.glb',                 maxDistance: 5          },
      { path: '/models/space_station_3/spacestation3_LOD3.glb',                 maxDistance: Infinity, animated: false },
    ];
    if (subtype === 'orbiting') return [
      { path: '/models/Sputnik satellite.glb', maxDistance: Infinity, animated: false },
    ];
    return []; // asteroid: no model, show fallback geometry
  }, [subtype]);

  const lodScale = subtype === 'deep' ? 0.01 : s * 1.5;
  const { bodyRef, modelLoaded } = useLodModels(lodEntries, groupRef, lodScale);
